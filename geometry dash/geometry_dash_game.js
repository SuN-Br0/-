...
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        function setPalisade(x , y, length) {   // кристаллики
            ctx.beginPath();
                ...
                ctx.fillRect(x*scale + map.x, (display_part*1/8+y)*scale, length*scale, scale/2);
            ctx.fill();
        }
        ...
        function setStep (x, y) {   // ступенька (маленький блок на половину сектора)
            ...
            ctx.fillStyle="#ff0000";
                ...
                ctx.fillRect(x*scale + map.x, (y+display_part*1/8)*scale - scale/3, scale, scale/3);
            ctx.fill();
        }
        ...
        function drawMap() {
            ...
            for (var i = 0; i < display_part + Math.floor(Math.abs(map.x_shift)/display_part); i++) {
                ctx.beginPath();
                    ctx.lineTo(i*scale + map.x , 0);
                    ctx.lineTo(i*scale + map.x , height);
                ctx.stroke();
            }
            ...
            ctx.beginPath();
                ...
                ctx.fillRect(0 + map.x,2*scale,scale, 14*scale); // стенка
...
// // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        //     var m = mouse_map(e);    // получаем координаты курсора
            
        //     map.x_shift = m.x - map.x_shift;  // начальное смещение - необходимо для адекватного перетаскивания канваса курсором
        ...
        //     map.x_shift = m.x - map.x_shift;  // перерасчитываем смещение
        ...
        //     map.x = m.x - map.x_shift;    // рассчет поправки координат объектов с учетом смещения
        //     drawMap();  // отрисовка всего необходимого
        // }

        // function mouse_map(e) {  // функция, которая вытаскивает координаты мыши
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        function drawMap() {
            ...
            for (var i = 0; i < display_part + Math.abs(map.x/32); i++) {    // вертикальные линии
                ctx.beginPath();
                    ctx.lineTo(i*scale + map.x , 0);
                    ctx.lineTo(i*scale + map.x , height);
                ctx.stroke();
    ...
    //        setFloor(30, 0, 10, 1);
        ...
        setSpike(32,0);
            ...
            ctx.beginPath();
                ctx.fillStyle = "#000000";
                ctx.fillRect(0, 0, width, bottom.height*scale);    // пол
                ctx.fillRect(0 + map.x, 1/8*display_part*scale, scale, 14*scale); // стенка
